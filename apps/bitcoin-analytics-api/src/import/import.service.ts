...
export class ImportService implements OnApplicationBootstrap {
      ...
      for (let i = 0; i < blockData.tx.length; i += 500) {
        ...
        for (const [index, tx] of filteredTransactions.entries()) {
          ...
          for (const input of vin) {
            const inputEntity = this.mapInputToEntity(
              input,
          ...
          for (const output of vout) {
            const outputEntity = this.mapOutputToEntity(
              output,
              transactionEntities[index],
            );
  ...
  async addAddressTransaction(
    ...
    const queryRunner =
      this.addressTransactionRepository.manager.connection.createQueryRunner();
    ...
    try {
      ...
      const updatedBalance =
        type === 'receive' ? currentBalance + amount : currentBalance - amount;

      const addressTransaction = queryRunner.manager.create(
        AddressTransactionEntity,
        {
          address,
